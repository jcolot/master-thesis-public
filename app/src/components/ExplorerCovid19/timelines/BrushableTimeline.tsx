...
const createTimeline = (options: any) => {
  const { data, brushedDomain, domain, height, width, onBrush, theme } = {
    data: [],
  ...
  const setup = (data) => {
    ...
    const update = (newData) => {
      ...
      const plot = Plot.plot({
        ...
        marks: [
          Plot.areaY(data, {
            x: (d) => d.start.getTime() + (d.end.getTime() - d.start.getTime()) / 2,
            y: "indicator",
            fill: theme === "LIGHT" ? "darkgray" : "lightgray",
            insetLeft: 0.2,
            insetRight: 0.2,
          }),
        ],
...
const BrushableTimeline = forwardRef(function BrushableTimeline({ data, height, domain, brushedDomain, onBrush, style }, fwdRef) {

...
function generateLegend(
  ...
  if (colorScale?.interpolate) {
    ...
    x = colorScale.copy().rangeRound(d3.quantize(d3.interpolate(marginLeft, width - marginRight), n));
  ...
  // Sequential
    ...
    x = Object.assign(colorScale.copy().interpolator(d3.interpolateRound(marginLeft, width - marginRight)), {
      range() {
...
function ColorLegend({ colorScale, title, tickFormat, tickSize = 6, width = 320 }) {
  const ref = useRef();
  useEffect(() => {
    ref.current.innerHTML = "";
    ref.current.append(generateLegend(colorScale, { title, tickFormat, tickSize, width }));
  }, [colorScale, title, tickFormat, tickSize, width]);
